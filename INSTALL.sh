...
prepare_host() {
	local hostdeps="libusb-dev git parted libz1 libstdc++6-arm64-cross libusb-0.1-4 libusb-1.0-0-dev libusb-1.0-0 ccache libncurses5-dev pv base-files linux-base"
	local deps=()
	local installed=$(dpkg-query -W -f '${db:Status-Abbrev}|${binary:Package}\n' '*' 2>/dev/null | grep '^ii' | awk -F '|' '{print $2}' | cut -d ':' -f 1)

	if [[ "$DISTRIB" == "Ubuntu" ]] && [[ "$DISTRIB_RELEASE" == "18.10" || "$DISTRIB_RELEASE" =~ "20" || "$DISTRIB_RELEASE" =~ "21" ]]; then
		hostdeps="$hostdeps libncurses6"
	else
		hostdeps="$hostdeps libncurses5-dev"
	fi
